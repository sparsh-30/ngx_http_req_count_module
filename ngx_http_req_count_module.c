...
ngx_http_req_count_timer_handler(ngx_event_t *ev)
{
    ...
    ngx_atomic_cmp_set(&ctx->count, old, 0);
...
ngx_http_req_count_init_process(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < mcf->zones->nelts; i++) {
        ...
        tctx->ctx = ctx;
        tctx->freq_ms = ctx->freq_ms;

        tctx->event.handler = ngx_http_req_count_timer_handler;
        tctx->event.log = cycle->log;
        tctx->event.data = tctx;

        ngx_add_timer(&tctx->event, tctx->freq_ms);
    }
